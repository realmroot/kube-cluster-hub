...
Kube Cluster Hub is a TypeScript control plane and Go Connector that publish a
credential-free multi-cluster catalog, proxy current-user Kubernetes
credentials, and expose an OAuth-protected Agent Resource Server. It is not an
identity provider and must not implement a parallel Kubernetes authorization
model.
...
- Agent requests require an audience-bound DPoP token and execute as an
