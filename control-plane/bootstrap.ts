...
export async function bootstrap(
  ...
  const dependencies: AppDependencies = {
    config,
    store,
    catalogUsers: new UserVerifier(identity.issuer, config.apiUrl, 'access'),
    kubernetesUsers: new UserVerifier(identity.issuer, config.uiClientId, 'id'),
    agents: new AgentVerifier(identity.issuer, config.apiUrl, store),
