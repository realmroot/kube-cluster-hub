...
For Agent access, kube-apiserver must also accept the audience of the Resource Server token forwarded by the Hub. When an AuthenticationConfiguration lists the shared human client audience and the Hub Resource Server audience, configure `audienceMatchPolicy: MatchAny`. The Hub verifies that the Agent token is issued for its own Resource Server; Kubernetes independently verifies that the same token contains an audience it accepts.
...
- Treat tunnels as part of the data path. Reject a tunnel implementation that buffers an open watch/log response even if ordinary Kubernetes requests succeed.
